...
type RestOf<T extends IContract | NewContract> = Omit<T, 'startAt' | 'endAt'> & {
  startAt?: string | null;
  endAt?: string | null;
};
...
export class ContractService {
  ...
  protected convertDateFromClient<T extends IContract | NewContract | PartialUpdateContract>(contract: T): RestOf<T> {
    return {
      ...contract,
      startAt: contract.startAt?.toJSON() ?? null,
      endAt: contract.endAt?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restContract: RestContract): IContract {
    return {
      ...restContract,
      startAt: restContract.startAt ? dayjs(restContract.startAt) : undefined,
      endAt: restContract.endAt ? dayjs(restContract.endAt) : undefined,
    };
  }
