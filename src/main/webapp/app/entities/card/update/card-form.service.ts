...
type CardFormGroupContent = {
  ...
  token: FormControl<ICard['token']>;
  auth: FormControl<ICard['auth']>;
  key: FormControl<ICard['key']>;
  account: FormControl<ICard['account']>;
};
...
export class CardFormService {
  createCardFormGroup(card: CardFormGroupInput = { id: null }): CardFormGroup {
    ...
    return new FormGroup<CardFormGroupContent>({
      ...
      auth: new FormControl(cardRawValue.auth),
      key: new FormControl(cardRawValue.key),
      account: new FormControl(cardRawValue.account),
    });
  }
