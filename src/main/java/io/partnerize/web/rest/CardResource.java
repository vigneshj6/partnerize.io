...
/**
 * REST controller for managing {@link io.partnerize.domain.Card}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class CardResource {
        ...
        Optional<Card> result = cardRepository
            ...
            .map(existingCard -> {
                ...
                if (card.getAuth() != null) {
                    existingCard.setAuth(card.getAuth());
                }
                if (card.getKey() != null) {
                    existingCard.setKey(card.getKey());
                }
     ...
     */
    @GetMapping("/cards")
    public List<Card> getAllCards(@RequestParam(required = false) String filter) {
        if ("partner-is-null".equals(filter)) {
            log.debug("REST request to get all Cards where partner is null");
        ...
        if ("company-is-null".equals(filter)) {
            log.debug("REST request to get all Cards where company is null");
            return StreamSupport
                .stream(cardRepository.findAll().spliterator(), false)
